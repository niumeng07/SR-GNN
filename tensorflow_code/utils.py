...
def data_masks(all_usr_pois, item_tail):
    us_lens = [len(upois) for upois in all_usr_pois] #每个sess下click_items的个数
    len_max = max(us_lens)
    #在当前sess对应的click_items后面补0(由item_tail传入),达到最大长度
    us_pois = [upois + item_tail * (len_max - le) 
               for upois, le in zip(all_usr_pois, us_lens)]
    #最终形成的Masks,前K个值为1,后面的值为0(0<K<=N-1)
...
class Data():
    ...
    def generate_batch(self, batch_size):
        ...
        n_batch = int(self.length / batch_size) #总条数/batch_size得batch数目
        if self.length % batch_size != 0:
            n_batch += 1
        #slices是index无实际数据list[ndarray], list_length=n_batch, ndarray=[index]
